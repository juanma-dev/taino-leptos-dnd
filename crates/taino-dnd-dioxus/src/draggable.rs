...
use dioxus::prelude::*;
use taino_dnd_core::{
    transition, AnnounceEvent, Direction, DragEvent, DragState, DraggableId, DroppableId, Point,
    Rect, DEFAULT_DRAG_THRESHOLD,
};
    ...
    pub fn on_key_down(mut self, ev: Event<KeyboardData>) {
        ...
        if is_activation(&key) {
            ...
            if let Ok(state) = transition(current, DragEvent::PointerUp, DEFAULT_DRAG_THRESHOLD) {
                ...
                self.ctx
                    .announce_event(AnnounceEvent::Dropped { draggable: self.id, over: target });
            }
            return;
        }
